...
class ProcessManager(RemoteBase):
    def __init__(self, ip, pwd, controller=None):
        ...
        self.btn_find = QPushButton("FIND"); self.btn_find.setFixedWidth(70); self.btn_find.clicked.connect(self.load)
        self.btn_refresh = QPushButton("REFRESH"); self.btn_refresh.setFixedWidth(70); self.btn_refresh.clicked.connect(self.refresh_all)
        ...
        btns = QHBoxLayout(); self.auto_cb = QCheckBox("Auto-reload (5s)"); self.auto_cb.setChecked(True)
        b2 = QPushButton("KILL PROCESS"); b2.clicked.connect(self.kill); b2.setStyleSheet("background-color: #f44336; color: white; font-weight: bold;")
        btns.addWidget(self.auto_cb); btns.addStretch(); btns.addWidget(b2); layout.addLayout(btns)
        self.timer = QTimer(self); self.timer.timeout.connect(self.auto_load); self.timer.start(5000); self.load()

    def auto_load(self):
        if self.auto_cb.isChecked() and self.isVisible(): self.load()
    def refresh_all(self):
        self.search.clear(); self.load()
