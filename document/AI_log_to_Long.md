...
**23.** "Tại sao khi tôi thực hiện các lệnh nặng như tải file hoặc quay phim màn hình, giao diện ứng dụng Client của tôi lại bị 'đơ' (Not Responding)? Tôi vẫn muốn nhấn được các nút khác trong khi đang tải dữ liệu."
...
**24.** "Tôi gặp lỗi khi gửi danh sách file quá dài. Client đôi khi chỉ nhận được một nửa danh sách rồi dừng lại, hoặc nhận lẫn lộn dữ liệu của lệnh này với lệnh kia. Làm sao để Client biết chính xác khi nào một gói tin kết thúc?"
...
**25.** "Tôi đã chạy được Keylogger nhưng log trả về chỉ là các phím bấm liên tục, rất khó theo dõi. Làm sao để tôi biết được phím đó gõ trên ứng dụng nào (ví dụ Facebook hay Notepad)?"
...
**26.** "Tôi muốn quay phim màn hình Server rồi lưu lại thành file `.mp4`. Tôi đã chụp được chuỗi ảnh liên tiếp nhưng làm sao để ghép chúng lại thành video và gửi về Client?"
...
**27.** "Thỉnh thoảng mạng của tôi bị chập chờn khiến kết nối giữa Client và Server bị đứt. Mỗi lần như vậy tôi lại phải tắt app đi bật lại rất phiền. Có cách nào để app tự động kết nối lại không?"
...
**28.** "Tôi gặp vấn đề là khi Server đột ngột tắt (ví dụ tôi nhấn Ctrl+C ở máy Server), các cửa sổ con bên Client (như Live Control hay Files) vẫn mở. Nếu tôi lỡ tay nhấn vào các nút trên đó, ứng dụng Client sẽ bị crash ngay lập tức vì Socket đã chết. Làm sao để xử lý việc này một cách an toàn?"
...
**29.** "Mỗi lần mở ứng dụng tôi lại phải nhập IP và Mật khẩu của máy Server rất mất thời gian. Có cách nào để Client 'nhớ' những máy đã từng kết nối không?"
...
**30.** Giải quyết lỗi "ModuleNotFoundError" khi tách file như thế nào cho chuyên nghiệp?**
...
**31.** "Tôi muốn khi đang Live Stream màn hình vẫn có thể mở được bảng Process để tắt một ứng dụng đang treo. Hiện tại tôi chỉ dùng 1 socket nên nếu đang stream ảnh thì không gửi lệnh khác được. Tôi phải cấu trúc lại thế nào?"
...
**32.** Làm sao để dừng hoàn toàn các luồng ngầm (Background Threads) khi tắt ứng dụng?**
...
**33.** "Tôi sử dụng writer.release() và khi tôi nhấn 'Stop Record', tôi muốn Server gửi ngay file video về cho Client. Tuy nhiên, đôi khi file gửi về bị lỗi không mở được, hoặc dung lượng file nhận được là 0 byte. Tôi đã sai ở đâu?"
...
**34.** "Nếu trong quá trình tôi đang điều khiển, người dùng ở máy Server thay đổi độ phân giải màn hình (ví dụ từ 1920x1080 xuống 1280x720), ứng dụng Client của tôi sẽ bị lệch tọa độ chuột hoặc bị lỗi hiển thị. Làm sao để xử lý tình huống 'nóng' này?"
...
**35.** "Tôi đang làm phần Activity Logs để theo dõi phím bấm. Cứ mỗi 2 giây Client lại tự động nạp log mới từ Server. Tuy nhiên, có một trải nghiệm rất tệ là mỗi khi có log mới, thanh cuộn (Scrollbar) lại tự động nhảy xuống cuối, làm tôi không thể tập trung đọc những dòng ở giữa được. Làm sao để xử lý việc này?"
...
**36.** "Tôi muốn thêm tính năng Tắt máy (Shutdown) và Khởi động lại (Restart) máy Server. Nhưng tôi lo lắng là nếu người dùng lỡ tay nhấn nhầm nút trên giao diện thì máy Server sẽ sập ngay lập tức, rất nguy hiểm. Tôi nên thiết kế bước xác nhận như thế nào?"
...
**37.** "Tôi muốn Dashboard hiển thị trạng thái của Server (Online/Offline) theo thời gian thực. Tôi có nên dùng một vòng lặp `while True` để liên tục kiểm tra không? Tôi sợ nó sẽ làm treo toàn bộ ứng dụng."
...
**38.** Tôi dùng Thread rồi nhưng tại sao khi cập nhật ảnh vào QLabel lại bị crash app?**
...
**39.** Tại sao ứng dụng lại bị "Not Responding" mỗi khi tôi bắt đầu Live Stream?**
...
**40.** Thư viện `mss` thực sự nhanh hơn `pyautogui` đến mức nào? Tại sao?**
...
**41.** Tại sao hàm `recv(1024)` lại thường xuyên gây ra lỗi vỡ ảnh khi truyền qua mạng?**
...
**42.** "Tôi thấy việc cài đặt các thư viện như `PyQt5`, `opencv`, `psutil` khá mất thời gian và dễ bị lỗi xung đột phiên bản giữa các máy. Có cách nào để tôi đóng gói danh sách các thư viện này chỉ cần chạy một lệnh duy nhất là cài được toàn bộ không?"
...
**43.** "Tôi muốn không cần phải cài đặt Python hay chạy lệnh terminal mà chỉ cần click vào một file `.exe` (trên Windows) hoặc một file thực thi để xem demo. Python có hỗ trợ đóng gói toàn bộ dự án thành một file duy nhất không?"
...
**44.** "Tôi đang phân vân giữa việc dùng TCP hay UDP để truyền hình ảnh màn hình. Tôi nghe nói UDP nhanh hơn và hay được dùng cho livestream, nhưng TCP lại đảm bảo dữ liệu không bị mất. Đối với một ứng dụng điều khiển từ xa, lựa chọn nào là tối ưu hơn?"
...
**45.** "Ứng dụng Server của tôi cần chạy liên tục trong nhiều giờ, thậm chí nhiều ngày. Tôi sợ rằng theo thời gian, ứng dụng sẽ chiếm dụng bộ nhớ RAM ngày càng nhiều (Memory Leak) và làm treo hệ thống. Tôi nên chú ý điều gì khi viết code Server?"
...
**46.** "Tôi đang gặp một lỗi rất khó hiểu trên Ubuntu 24.04: khi chạy ứng dụng thì báo lỗi 'Could not load the Qt platform plugin xcb', mặc dù tôi đã cài đủ thư viện. Sau khi tìm hiểu, tôi thấy có sự xung đột giữa OpenCV và PyQt5. Bạn có thể giải thích rõ hơn và đưa ra giải pháp triệt để không?"
...
**47.** "Trong Dashboard, tôi muốn hiển thị địa chỉ IP thực tế của máy Server để người dùng dễ dàng kết nối. Tuy nhiên, nếu dùng `socket.gethostbyname(socket.gethostname())`, tôi thường chỉ nhận được địa chỉ localhost `127.0.0.1`. Làm cách nào để lấy được IP mạng LAN (ví dụ 192.168.x.x) một cách chính xác nhất?"
...
**48.** "Tôi muốn tính năng Live Control trở nên chuyên nghiệp hơn bằng cách hỗ trợ kéo thả chuột (Mouse Drag) để di chuyển cửa sổ hoặc bôi đen văn bản trên máy Server. Tôi nên bắt các sự kiện chuột như thế nào trên PyQt5 để truyền tải hành động này mượt mà nhất?"
...
**49.** "Tôi thấy dự án của mình bắt đầu có nhiều thư viện và file cấu hình. Bạn khuyên tôi nên dùng `pyproject.toml` thay vì `requirements.txt`. Cấu trúc này mang lại lợi ích gì cho việc quản lý lâu dài và bảo trì đồ án sau này?"
...
**50.** "Các phím tổ hợp hệ thống như Alt+Tab, Ctrl+C hay phím Windows thường bị máy Client đánh chặn mất thay vì gửi sang máy Server. Làm sao để tôi có thể bắt được các phím này và thực thi chúng trên máy từ xa?"
...
**51.** "Khi truyền tải các file lớn hoặc video ghi hình màn hình nặng hàng trăm MB, người dùng sẽ cảm thấy lo lắng nếu màn hình cứ đứng yên. Tôi muốn thêm một thanh tiến trình (Progress Bar) động. Làm sao để tính toán phần trăm và cập nhật nó theo thời gian thực mà không làm chậm quá trình nhận dữ liệu?"
...
**52.** "Dashboard hiện tại của tôi chỉ hiện danh sách máy chủ. Tôi muốn nâng cấp nó thành một trung tâm điều khiển đa nhiệm, nơi tôi có thể quản lý và giám sát trạng thái của nhiều Server cùng lúc. Bạn gợi ý cấu trúc dữ liệu và giao diện như thế nào?"
...
**53.** "Tôi muốn đảm bảo rằng khi tôi đóng Dashboard (cửa sổ chính), toàn bộ các kết nối mạng và các cửa sổ con đang mở phải được dọn dẹp sạch sẽ để tránh tình trạng treo tiến trình ngầm. Tôi nên sử dụng cơ chế nào của PyQt5 để thực hiện việc này?"
...
**54.** "Tôi có một vấn đề tò mò, như bạn biết 1 máy tính có thể có nhiều ổ đĩa, làm cách nào để tôi có thể scan hết ứng dụng và file?"                                                                                                                                                                                                                                              
...
**55.** "Tôi đã cố gắng nghiên cứu trên YouTube và Google về cách làm tính năng Live Control (điều khiển màn hình từ xa) nhưng thấy kiến thức rất rời rạc và khó áp dụng. Bạn có gợi ý nào về một quy trình chuẩn để xử lý vấn đề này không?"
...
**56.** "Làm cách nào để bắt được toàn bộ các sự kiện từ bàn phím và chuột trong tính năng Live Control?"
...
**57.** "Cách xử lý các sự kiện chuột và bàn phím trong Live Control để đảm bảo độ chính xác về tọa độ và các tổ hợp phím phức tạp?"
...
**58.** "Kiến trúc để tạo ra một tính năng Live Control mượt mà và không gây treo ứng dụng là gì?"
...
**59.** "Chúng tôi đang xây dựng ứng dụng điều khiển từ xa tương tự TeamViewer. Với yêu cầu về độ trễ thấp cho Live Monitor nhưng phải đảm bảo an toàn cho các lệnh điều khiển, chúng tôi nên sử dụng TCP hay UDP? Và làm sao để tích hợp bảo mật mà không làm chậm hệ thống?"

** TCP là lựa chọn an toàn nhất cho các lệnh (Command) và truyền file nhờ tính tin cậy. Tuy nhiên, để tối ưu hình ảnh, nhóm nên dùng TCP kết hợp nén JPEG qua OpenCV. Về bảo mật, thay vì tự viết mã hóa, hãy dùng thư viện `ssl` tiêu chuẩn để bọc Socket. Điều này vừa đảm bảo an toàn (TLS 1.3), vừa tận dụng được tối ưu phần cứng của CPU cho việc mã hóa.

**60.** "Khi chạy trên Ubuntu 24.04, chúng tôi gặp lỗi 'Could not load the Qt platform plugin xcb'. Ngoài ra, việc dùng OpenCV chung với PyQt5 dường như gây xung đột thư viện đồ họa. Có giải pháp nào xử lý triệt để mà không yêu cầu người dùng cài đặt quá nhiều thư viện hệ thống?"

** Đây là vấn đề xung đột `libQt` giữa OpenCV và PyQt5 trên các bản Linux mới. Giải pháp chuẩn công nghiệp là chuyển sang dùng `opencv-python-headless` (phiên bản không có GUI). Đồng thời, trong mã nguồn cần thiết lập biến môi trường `QT_QPA_PLATFORM="xcb"` ngay trước khi khởi tạo `QApplication` để ép hệ thống sử dụng đúng plugin cửa sổ.

**61.** "Chúng tôi gặp hiện tượng 'UI Freeze' (treo giao diện) khi tải các tệp tin lớn qua Socket SSL. Mặc dù đã dùng QThread nhưng việc cập nhật Progress Bar dường như vẫn bị nghẽn. Làm sao để luồng giao diện vẫn mượt mà trong khi luồng mạng đang nhận hàng GB dữ liệu?"

** Vấn đề nằm ở việc 'Blocking I/O'. Nhóm nên triển khai cơ chế 'Non-blocking read' hoặc sử dụng vòng lặp nhận dữ liệu theo khối (chunks) nhỏ (ví dụ 64KB). Sau mỗi chunk, hãy gọi `QApplication.processEvents()` (nếu không dùng thread) hoặc phát Signal về UI. Đặc biệt, cần chú ý lỗi Overflow nếu truyền kích thước byte trực tiếp vào Progress Bar, hãy chuyển sang tính phần trăm (0-100) để đảm bảo an toàn.

**62.** "Để ứng dụng có thể điều khiển nhiều Server cùng lúc từ một Client duy nhất, cấu trúc Command-Response thông thường dường như không còn hiệu quả vì dữ liệu sẽ bị chồng chéo trên một đường truyền. Chúng tôi đang cân nhắc kiến trúc Multi-Connection, bạn nghĩ sao?"

** Đó là tư duy rất tốt. Hãy triển khai cơ chế **Session Controller**. Mỗi khi mở một cửa sổ tính năng (Live Control, File Explorer), Client sẽ khởi tạo một kết nối SSL mới tới Server. Trên Server, hãy dùng `threading` để xử lý mỗi connection này độc lập. Cách tiếp cận này giúp tách biệt hoàn toàn luồng lệnh và luồng dữ liệu nặng, đảm bảo tính đa nhiệm thực thụ cho ứng dụng.

**63.** "Làm sao để ánh xạ (map) tọa độ chuột chính xác khi màn hình Client nhỏ hơn màn hình Server, đồng thời hỗ trợ cả việc Server không có màn hình vật lý (Headless)?"

** Nhóm cần thực hiện phép tính tỉ lệ (Ratio Mapping). Lấy độ phân giải thực của Server (request từ hệ thống) và kích thước `QLabel` hiển thị trên Client. Công thức: `X_real = X_click * (Server_W / Label_W)`. Với trường hợp Headless, hãy dùng `try-except` khi chụp màn hình và trả về một Frame giả lập (ví dụ ảnh đen 1280x720) để ứng dụng không bị crash và người dùng vẫn biết trạng thái máy từ xa.
